...
# 2025
#
# Description: SRU_LSTM_Gated implementation - LSTM with Additive Transformation
# and Gated Refinement mechanism inspired by the Refine Gate technique.

import torch
import torch.nn as nn
from typing import Optional, Tuple

class LSTMSRUGateCell(nn.Module):
    def __init__(self, input_size, hidden_size, bias=True):
        super(LSTMSRUGateCell, self).__init__()
...
class LSTM_SRU_Gate(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=1, batch_first=False):
        super(LSTM_SRU_Gate, self).__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.batch_first = batch_first
        self.cells = nn.ModuleList([LSTMSRUGateCell(input_size if i == 0 else hidden_size, hidden_size) 
...
if __name__ == '__main__':
    ...
    model = LSTM_SRU_Gate(input_size, hidden_size, num_layers=2, batch_first=batch_first).to(device)
